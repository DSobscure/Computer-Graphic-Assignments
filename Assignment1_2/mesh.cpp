...
mesh::mesh(std::string fileDirectory, std::string obj_file)
{
	mTotal = 0;		// mList[0] reserved for default meterial
	vTotal = tTotal = nTotal = fTotal = 0;
	
	Init(fileDirectory, obj_file);
}
...
void mesh::LoadMesh(string fileDirectory, string obj_file)
{
	FILE	*scene;
	...
	scene = fopen((fileDirectory+"/"+obj_file).c_str(),"r");
	s_file = obj_file;
	...
	while(!feof(scene))
	{
		...
		else if (!strcmp(token,"mtllib"))
		{
			...
			LoadMtl(fileDirectory, string(mat_file));
		}
...
void mesh::LoadMtl(string fileDirectory, string tex_file)
{
	char	token[100], buf[100];
	float	r,g,b;

	fp_mtl = fopen((fileDirectory+"/"+tex_file).c_str(),"r");
...
void mesh::Init(std::string fileDirectory, std::string obj_file)
{
	...
	LoadMesh(fileDirectory, string(obj_file));		// 讀入 .obj 檔 (可處理 Material)
}
